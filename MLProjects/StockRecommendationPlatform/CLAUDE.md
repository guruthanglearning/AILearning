...
**LLM model selection** (`app/services/claude_service.py`): the analysis model is chosen per-request from the UI, not hardcoded — `DEFAULT_MODEL = "claude-opus-4-8"`. `claude-fable-5` and `claude-fable-5-1` opt into Anthropic's server-side fallback (`extra_body.fallbacks` + `betas=["server-side-fallback-2026-06-01"]`) so a policy *refusal* is re-served by `claude-opus-4-8` in the same call — this covers refusals specifically, not ordinary auth/connection/rate-limit failures, which still raise normally; other models don't have this fallback at all.
