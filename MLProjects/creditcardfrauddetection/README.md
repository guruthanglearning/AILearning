...
- There is no unified "learning pipeline." Two entirely separate, disconnected mechanisms exist:
  ...
  2. **Offline XGBoost training** — `scripts/manage_models.py` engineers features from a CSV, does an 80/20 `train_test_split` (not k-fold cross-validation, despite the old diagram's "Cross Validation" label), fits XGBoost, and saves to `data/models/*.joblib`. Its `prepare_data()` used to build a plain `dict` per row, but `engineer_features()` requires attribute access (`transaction.timestamp`, `.transaction_id`, `.customer_id`, etc.), so `--action train` raised `AttributeError` before training could happen — fixed by constructing a real `Transaction` per row instead. It's still never invoked by the live API or the feedback endpoint, even now that it works.
