...
graph TB
    ...
    subgraph "🧠 AI/ML Layer"
        ...
        XGB[XGBoost Classifier<br/>heuristic fallback if unfitted]
    ...
    subgraph "💾 Data Storage"
        ...
        MODELS[("data/models/*.joblib<br/>loaded via ML_MODEL_PATH/ML_SCALER_PATH")]
    ...
    HARDBLOCK -->|no| XGB
    XGB -->|"predict_proba (falls back to<br/>risk-factor heuristic if unfitted)"| DECISION
...
- `FraudDetectionService` now constructs `MLModel(model_path=settings.ML_MODEL_PATH, scaler_path=settings.ML_SCALER_PATH)` (both default to `data/models/fraud_model.joblib`/`scaler.joblib`), so the live API loads and uses the real trained XGBoost model — `predict_proba()` on real, differentiated feature vectors, not the heuristic fallback. `MLModel` still falls back to an in-memory, untrained demo model (and from there to the heuristic, exactly as before) if either file is missing or fails to load, so this remains safe for a fresh checkout with no trained model yet.
- The checked-in `data/models/fraud_model.joblib` is trained on `data/sample/synthetic_transactions_large.csv` (2,000 rows generated by `scripts/generate_synthetic_data.py`, a hand-written sampling rule — not real fraud data) via `scripts/manage_models.py --action train`. Treat its reported metrics (accuracy/AUC near-perfect) as a demonstration that the pipeline works end to end, not as a claim about real-world fraud-detection difficulty — the synthetic fraud/legitimate distributions are deliberately fairly separable.
...
sequenceDiagram
    ...
    alt Hard block (sanctioned country, or category mismatch + high-risk country)
        ...
        ML-->>Service: fraud_probability, ml_confidence (real XGBoost model, or heuristic fallback if unfitted/missing)
...
flowchart LR
    ...
    subgraph "🛠️ Offline Model Training (manual, but now wired up)"
    ...
    C4 -->|"loaded at startup via<br/>settings.ML_MODEL_PATH/ML_SCALER_PATH"| XGB[Live ML path]
...
- There is no unified "learning pipeline." Two entirely separate mechanisms exist:
  1. **Vector pattern store** — analyst feedback, bulk pattern ingestion, and admin-created patterns each add a new fraud-case document (embedded via HuggingFace, stored in Chroma/Pinecone, and chunked by `RecursiveCharacterTextSplitter` if long) that future `search_similar_patterns()` calls can retrieve. Nothing here retrains any model, and it stays fully disconnected from XGBoost training.
  2. **Offline XGBoost training** — `scripts/manage_models.py` engineers features from a CSV, does an 80/20 `train_test_split` (not k-fold cross-validation, despite the old diagram's "Cross Validation" label), fits XGBoost, and saves to `data/models/*.joblib`. It's still run manually (there's no endpoint or scheduled job that triggers it), but its output is no longer a dead end — `FraudDetectionService` now loads it at startup.
- `data/models/fraud_model.joblib` is trained on `data/sample/synthetic_transactions_large.csv` (2,000 rows generated by `scripts/generate_synthetic_data.py` — a hand-written sampling rule, not real fraud data). Treat it as a demonstration that the training→loading pipeline works end to end, not a production-quality fraud model.
- The `/metrics` endpoint (`get_model_metrics()`) still does **not** reflect this process — it generates simulated metrics (hardcoded baseline values with ±2% random jitter) and a randomized `last_trained` timestamp, not measurements from this training run.
